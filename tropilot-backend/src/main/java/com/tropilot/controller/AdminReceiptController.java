...
public class AdminReceiptController {
    ...
    @GetMapping
    public ApiResponse<List<ReceiptResponse>> getReceipts() {
        return ApiResponse.success("Receipts loaded successfully", receiptService.getReceipts());
    }

    @GetMapping("/{id}")
    public ApiResponse<ReceiptResponse> getReceipt(@PathVariable Long id) {
        return ApiResponse.success("Receipt loaded successfully", receiptService.getReceipt(id));
    }
}
